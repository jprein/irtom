export const translations = {
	beginning: {
		'german': '<h1 class="heading">Herzlich Willkommen</h1>',
		'english': '<h1 class="heading">Welcome</h1>',
	},
	introChild: {
		'german': '<p class="intro-text">Danke, dass du heute bei uns bist. Wir wollen ein kleines Spiel zusammen spielen. <br> Lass uns nun mehr über das Spiel anhören.</p>',
		'english': `<p class="intro-text">Thank you for joining us today. We want to play a little game together. Let's listen to more information about our game.</p>`,
	},
	introAdult: {
		'german': '<p class="intro-text">Erwachsene</p>',
		'english': '<p class="intro-text">Adult</p>',
	},

	thankYou: {
		'german': '<h1 class="heading">Vielen Dank!</h1>',
		'english': '<h1 class="heading">Thank you!</h1>',
	},
};
