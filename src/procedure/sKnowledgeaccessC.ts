...
export default async ({ currentSlide, previousSlide }) => {
	...
	const boyOrangeHandsUp = document.getElementById(`${slidePrefix}-boyorange-handsup`) as SvgInHtml;
	const boyOrangeHandsDown = document.getElementById(`${slidePrefix}-boyorange-handsdown`) as SvgInHtml;
	const boyBrownHandsUp = document.getElementById(`${slidePrefix}-boybrown-handsup`) as SvgInHtml;
	const boyBrownHandsDown = document.getElementById(`${slidePrefix}-boybrown-handsdown`) as SvgInHtml;
	const boxOpen = document.getElementById(`${slidePrefix}-box-open`) as SvgInHtml;
	const boxClosed = document.getElementById(`${slidePrefix}-box-closed`) as SvgInHtml;
	...
	await gsap
		.timeline()
		.to([boyOrangeHandsDown, boxClosed], {delay: 1.5, autoAlpha: 0, duration: 0.1})
		.to([boyOrangeHandsUp, boxOpen], {autoAlpha: 1, duration: 0.1, 
			onComplete: () => {
		...
		.to(boyOrangeHandsDown, {delay: 0.5, x: 1100, duration: 3, 
			onComplete: () => {
				play(`./communities/${data.community}/audio/${slidePrefix}-3.mp3`);
			}
		})
		.to(boyBrownHandsDown, {delay: 4, x: 0, duration: 3, 
			onComplete: () => {
		...
		.to(boyBrownHandsDown, {autoAlpha: 1, duration: 0.1, 
			onComplete: () => {
